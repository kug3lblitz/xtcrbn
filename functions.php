<?php
/**
 * xtcrbn functions and definitions
 *
 ...
 */
function xtcrbn_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on xtcrbn, use a find and replace
		* to change 'xtcrbn' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'xtcrbn', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'xtcrbn' ),
		)
	);
...
add_action( 'after_setup_theme', 'xtcrbn_setup' );
 ...
 */
function xtcrbn_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'xtcrbn_content_width', 640 );
}
add_action( 'after_setup_theme', 'xtcrbn_content_width', 0 );
 ...
 */
function xtcrbn_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'xtcrbn' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'xtcrbn' ),
...
function xtcrbn_scripts() {
	...
	wp_enqueue_style( 'Audiowide', 'https://fonts.googleapis.com/css2?family=Audiowide&display=swap' );
	...
	wp_enqueue_script( 'waypoints', 'https://cdnjs.cloudflare.com/ajax/libs/waypoints/4.0.1/noframework.waypoints.min.js' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'xtcrbn_scripts' );
